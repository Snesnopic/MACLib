...
public:
    ...
    Get an argument
    **************************************************************************************************/
    void GetArgument(int nArgument, APE::CSmartPtr<str_utfn> & rspString) const
    {
    ...
    /**************************************************************************************************
    Get threads argument
    **************************************************************************************************/
    bool GetThreads(int nArgument, int & rnThreads) const
    {
        // get the threads argument
        CSmartPtr<str_utfn> spThreads;
        GetArgument(nArgument, spThreads);
...
static void HandleAuto(CSmartPtr<str_utfn> & rspInputFilename, CSmartPtr<str_utfn> & rspOutputFilename, bool bAPEType)
{
    ...
    {
        // build the output filename
        rspOutputFilename.Assign(new str_utfn [APE_MAX_PATH], true);
...
{
    // variable declares
    CSmartPtr<str_utfn> spInputFilename; CSmartPtr<str_utfn> spOutputFilename;
    int nRetVal = ERROR_UNDEFINED;
    ...
    // store the filenames
    Arguments.GetArgument(1, spInputFilename);
    Arguments.GetArgument(2, spOutputFilename);
    ...
    CSmartPtr<str_utfn> spMode;
    Arguments.GetArgument(2, spMode);

    if ((wcsncmp(spMode, L"-v", 2) != 0) &&
        (wcsncmp(spMode, L"-V", 2) != 0) &&
        (_wcsnicmp(spMode, L"-t", 2) != 0) &&
        (_wcsnicmp(spMode, L"-r", 2) != 0) &&
        (_wcsnicmp(spMode, L"-L", 2) != 0))
    {
        // modes other than verify, tag, remove tag, and legacy use at least the third argument
        if (argc < 4)
        {
            DisplayProperUsage(stderr);
            exit(-1);
        ...
        // get the mode (since it will be the next argument in these cases)
        Arguments.GetArgument(3, spMode);
    }

    // get the mode
    nMode = UNDEFINED_MODE;
    if (_wcsnicmp(spMode, L"-c", 2) == 0)
        nMode = COMPRESS_MODE;
    else if (_wcsnicmp(spMode, L"-d", 2) == 0)
        nMode = DECOMPRESS_MODE;
    else if (wcsncmp(spMode, L"-V", 2) == 0)
        nMode = VERIFY_FULL_MODE;
    else if (wcsncmp(spMode, L"-v", 2) == 0)
        nMode = VERIFY_MODE;
    else if (_wcsnicmp(spMode, L"-n", 2) == 0)
        nMode = CONVERT_MODE;
    else if (_wcsnicmp(spMode, L"-r", 2) == 0)
        nMode = REMOVE_TAG_MODE;
    else if (_wcsnicmp(spMode, L"-t", 2) == 0)
        nMode = TAG_MODE;
    else if (_wcsnicmp(spMode, L"-L", 2) == 0)
    ...
    {
        // get the threads argument
        if (Arguments.GetThreads(4, nThreads))
            nTagArgument = 6;
    }
    else if (nMode == DECOMPRESS_MODE)
    {
        // get the threads argument
        Arguments.GetThreads(4, nThreads);
    }
    else if ((nMode == VERIFY_FULL_MODE) || (nMode == VERIFY_MODE))
    {
        // get the threads argument
        Arguments.GetThreads(3, nThreads);
    }
    ...
    if (nMode == COMPRESS_MODE)
    {
        ...
        // tag
        if ((nRetVal == ERROR_SUCCESS) && (argc > nTagArgument))
        {
            APE::CSmartPtr<str_utfn> spTagSwitch;
            Arguments.GetArgument(nTagArgument - 1, spTagSwitch);
            if ((spTagSwitch.GetPtr() != NULL) && (_wcsicmp(spTagSwitch, L"-t") == 0))
            {
                fwprintf(stderr, L"\nTagging...\n");

                CSmartPtr<str_utfn> spTag;
                Arguments.GetArgument(nTagArgument, spTag);
