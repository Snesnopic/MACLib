    ...
    if (bSkip == false)
    {
        ...
        if (bMakesOutput && (nRetVal == ERROR_SUCCESS))
        {
            ...
            else if ((nRetVal == ERROR_SUCCESS) && (pInfo->GetOutputExtension() == _T(".ape")) && (pInfo->m_Tag.m_bFoundTag != false))
            {
                ...
                if (pInfo->m_Tag.m_nTrackNumber != -1)
                {
                    CString strTrackNumber;
                    strTrackNumber.Format(_T("%d"), pInfo->m_Tag.m_nTrackNumber);
                    spAPETagNew->SetFieldString(APE_TAG_FIELD_TRACK, strTrackNumber);
                }
                if (pInfo->m_Tag.m_nDiscNumber != -1)
                {
                    CString strDiscNumber;
                    strDiscNumber.Format(_T("%d"), pInfo->m_Tag.m_nDiscNumber);
                    spAPETagNew->SetFieldString(APE_TAG_FIELD_DISC, strDiscNumber);
                }
